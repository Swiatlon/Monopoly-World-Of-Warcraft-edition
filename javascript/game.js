...
class Game {
  ...
  gameMechanism(thisPlayer) {
    ...
    let emptyFieldWithoutOwner =
      Cities[thisPlayer.field].ownerOfField === undefined && Cities[thisPlayer.field].specialField !== true;
    let playerField =
      Cities[thisPlayer.field].ownerOfField === thisPlayer && Cities[thisPlayer.field].specialField === false;
    let otherPlayerField =
      Cities[thisPlayer.field].ownerOfField !== thisPlayer && Cities[thisPlayer.field].specialField !== true;
    ...
    switch (true) {
      ...
      case playerField: // my field
        map.showingDivs(containerOfBuyingHouses);
        for (let i = 0; i <= Cities[thisPlayer.field].houses; i++) {
          // Functions which disabled buying houses that we have
          checkboxesCounterOfHouses[i].checked = true;
        }
        for (let i = Cities[thisPlayer.field].houses; i <= checkboxesCounterOfHouses - 1; i++) {
          // Functions which give us chance to buy  a house
      ...
      case otherPlayerField:
        ...
        if (
          thisPlayer.money <
          Cities[thisPlayer.field].tribute * Cities[thisPlayer.field].eventMultiplier * multiplierMoney
        ) {
          ...
          neededGoldInSellingBuildings.textContent =
            'Potrzebny gold: ' +
            Cities[thisPlayer.field].tribute * multiplierMoney * Cities[thisPlayer.field].eventMultiplier;
          ...
          thisPlayer.money -=
            Cities[thisPlayer.field].tribute * multiplierMoney * Cities[thisPlayer.field].eventMultiplier;
          Cities[thisPlayer.field].ownerOfField.money +=
            Cities[thisPlayer.field].tribute * multiplierMoney * Cities[thisPlayer.field].eventMultiplier;
      ...
      case thisPlayer.field == jail:
        ...
        console.log(
          `[LICZNIK SIEDZENIA W WIEZIENIU GRACZA] + ${thisPlayer.nameOfPlayer}`,
          thisPlayer.counterOfStayingInJail
        );
        break;
      case Cities[thisPlayer.field].specialField === true:
        if (thisPlayer.field == 24) {
          ...
          switch (randomNumber) {
            ...
            case 2:
              text.textContent =
                'Na jedno z twoich miast spadła bomba many i zniszczyła twoje domki.Jesli nie posiadasz domkow ta karta nie zadziala';
              ...
              containerOfCards.children[0].onclick = function () {
                if (thisPlayer.cities.length > 0) {
                  ...
                  map.allLands[choosedFieldToDelete.field].children[1].firstElementChild.firstElementChild.src =
                    'data:,';
                  map.allLands[
                    choosedFieldToDelete.field
                  ].children[1].firstElementChild.firstElementChild.style.display = 'none';
                } else {
            ...
            case 5:
              text.textContent =
                'Obrabowaly cię pobliskie gobliny tracisz 200 golda bądz sie zeruje jesli miales tylko 200 golda';
              containerOfCards.children[0].onclick = function () {
              ...
              firstButton.onclick = function () {
                ...
                inputArray.forEach(function (target) {
                  target.addEventListener('click', function () {
                    ...
                    map.allLands[
                      target.offsetParent.offsetParent.classList[0]
                    ].children[1].firstElementChild.firstElementChild.src = 'data:,';
                    map.allLands[
                      target.offsetParent.offsetParent.classList[0]
                    ].children[1].firstElementChild.firstElementChild.style.display = 'none';
                    arrayOfTributeFields[target.offsetParent.offsetParent.classList[0]].textContent =
                      Cities[target.offsetParent.offsetParent.classList[0]].tribute *
                      Cities[target.offsetParent.offsetParent.classList[0]].multiplierDependFromHouses *
                      Cities[target.offsetParent.offsetParent.classList[0]].eventMultiplier;
          ...
          switch (
            randomNumber // there is switch statement with appending childs
          ) {
            case 6:
    ...
    if (thisPlayer.field !== 24) {
      //  when player is on field teleport button must be blocked
      btn.disabled = false;
    }
  }
  ...
  gettingMultiplierMoney(amountOfHouses) {
    let multiplierMoney;
    switch (
      amountOfHouses // tribute * multiplierMoney
    ) {
      case -1:
        multiplierMoney = 1;
        break;
  ...
  listenerHowMuchHouses(thisPlayer) {
    ...
    for (let i = amountOfHouses; i < checkboxesCounterOfHouses.length; i++) {
      checkboxesCounterOfHouses[i].addEventListener('click', function () {
        ...
        for (let i = amountOfHouses; i < checkboxesCounterOfHouses.length; i++) {
          // reseting function to only be clicked one button
          checkboxesCounterOfHouses[i].checked = false;
        }
  ...
  buyingHouses(thisPlayer) {
    if (playerPick !== undefined) {
      ...
      if (thisPlayer.money >= requirement) {
        if (Cities[thisPlayer.field].houses === -1) {
          // Field is empty
          thisPlayer.money -=
            Cities[thisPlayer.field].costOfTheField + Cities[thisPlayer.field].costOfOneHouse * playerPick;
        } else if (Cities[thisPlayer.field].houses > -1) {
          thisPlayer.money -=
            Cities[thisPlayer.field].costOfOneHouse * playerPick -
            Cities[thisPlayer.field].houses * Cities[thisPlayer.field].costOfOneHouse;
        }
        if (playerPick === 0) {
          ...
          map.allLands[
            thisPlayer.field
          ].children[1].firstElementChild.firstElementChild.src = `images/${thisPlayer.color}${playerPick}Houses.png`;
  ...
  deletingEverythingThatPlayerHad() {
    ...
    for (let i = 0; i < map.allLands[this.players[playerQueue].field].children[0].children.length; i++) {
      ...
      if (
        map.allLands[this.players[playerQueue].field].children[0].children[i].imgIdentyficator ==
        game.players[playerQueue].nameOfPlayer
      ) {
        map.allLands[this.players[playerQueue].field].children[0].removeChild(
          map.allLands[this.players[playerQueue].field].children[0].children[i]
        );
        console.log('[BYLEM]');
      }
    }
...
animated.addEventListener('transitionend', function () {
  if (flag === true) {
    // cube.js  26th line  flage only one times execute
...
surrenderString.addEventListener('click', () => {
  Cities[game.players[playerQueue].field].ownerOfField.money +=
    Cities[game.players[playerQueue].field].tribute *
    Cities[game.players[playerQueue].field].multiplierDependFromHouses *
    Cities[game.players[playerQueue].field].eventMultiplier;
